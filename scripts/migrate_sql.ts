...
async function migrate() {
    try {
        ...
        const adminResult = await db.execute(sql`SELECT id FROM users WHERE email = 'admin@pitzi.com.br' LIMIT 1`);
        const admin = adminResult.rows[0];

        if (!admin) {
            console.error("❌ Usuário admin@pitzi.com.br não encontrado.");
            process.exit(1);
        }
